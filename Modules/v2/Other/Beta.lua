...
local vflyBG = nil
local vflyUI = nil
local vflyMoveState = 0 -- 0: Stop, 1: Forward, -1: Backward

local function stopVFly()
    if vflyConnection then vflyConnection:Disconnect() end
    vflyConnection = nil
    if vflyBV then vflyBV:Destroy() end
    if vflyBG then vflyBG:Destroy() end
    vflyBV = nil
    vflyBG = nil
    if vflyUI then vflyUI:Destroy() vflyUI = nil end
end

local function createVFlyButtons()
    if vflyUI then vflyUI:Destroy() end
    vflyUI = Instance.new("ScreenGui")
    ...
    vflyConnection = RunService.RenderStepped:Connect(function()
        ...
        if vflyMode == "Joystick" then
            ...
            if moveVector.Magnitude > 0.1 then
                ...
                vflyBV.Velocity = direction.Unit * speed
            else
                vflyBV.Velocity = Vector3.zero
            end
        else
            if vflyMoveState == 1 then
                vflyBV.Velocity = camera.CFrame.LookVector * speed
...
RunService.RenderStepped:Connect(function()
	for plr, data in pairs(drawings) do
		...
		if Toggles.ESP and Toggles.ESP.Value and data.billboard and data.label and onScreen and isPlayerAllowed(plr, Options.ESPTeamOnly, Options.ESPPlayersOnly) then
			...
			data.label.Text = plr.Name .. " [" .. math.floor(dist) .. "]"
		else
			if data.billboard then data.billboard.Enabled = false end
		end
...
ESPConfigTab:AddDropdown("ESPFont", {
	Text="ESP Font", 
	Values={"BuilderSans","SourceSans","Roboto","Arcade"}, 
	Default=1, 
	Callback=function(v) 
...
local function IsValidTarget(plr)
    ...
    if Options.IgnoreTeam.Value then
        for teamName, ignored in pairs(Options.IgnoreTeam.Value) do
             if ignored and plr.Team and plr.Team.Name == teamName then return false end
        end
    end
...
local function GetClosestPlayer()
    ...
    for _, plr in Players:GetPlayers() do
        if IsValidTarget(plr) then
            ...
            if not isPrioritized and Options.PrioritizeTeam.Value then
